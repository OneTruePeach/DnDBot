...
const { createAudioResource, entersState, StreamType, AudioPlayerStatus, getVoiceConnection, getVoiceConnections } = require('@discordjs/voice');
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    console.log(`${new Date().toLocaleString()} - Main     | Changing BGM to ${songName} (${song}) in ${guildInfo.Name}`);

    canPlay = await sessionHandler.execute(interaction, guildInfo, true, false);
    vcConnections = getVoiceConnections("main");
    vcConnectionToGuild = vcConnections.get(guildInfo.Id);
    vcConnection = vcConnectionToGuild;

    if ( song == 'pause' ) {
        interaction.reply({ content:`Pausing BGM.`, flags: MessageFlags.Ephemeral });
    ...
    if ( song == 'play' ) {
      interaction.reply({ content:`Unpausing BGM.`, flags: MessageFlags.Ephemeral });
